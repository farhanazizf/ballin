...
export function MatchesClient({
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    if (!teamId || !opponent.trim() || !playedAt) {
      setError('Isi kelas, lawan, dan tanggal pertandingan.');
      return;
    }
    startTransition(async () => {
      ...
      if (!res.ok) {
        setError(body.error ?? 'Gagal menyimpan pertandingan.');
        return;
      }
      setShowForm(false);
  ...
  return (
    <div className="min-h-[100dvh] px-4 py-6 md:px-8 md:py-8">
      <header className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--color-report-text)] font-[family-name:var(--font-display)]">Pertandingan</h1>
          <p className="text-sm text-[var(--color-report-text-2)] mt-1">Jadwalkan match dan catat box score</p>
        </div>
        <Button variant="report-primary" size="sm" onClick={() => setShowForm((v) => !v)}>
          <Plus size={16} weight="bold" /> Match baru
        </Button>
      </header>
      ...
      {showForm ? (
        <form onSubmit={handleCreate} className="border-2 border-[var(--color-report-border)] bg-[var(--color-report-surface)] p-5 mb-6 space-y-4">
          ...
          <input value={opponent} onChange={(e) => setOpponent(e.target.value)} placeholder="Nama lawan" className="h-12 w-full px-4 border border-[var(--color-report-border)]" />
          ...
          <input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Lokasi (opsional)" className="h-12 w-full px-4 border border-[var(--color-report-border)]" />
          <Button type="submit" variant="report-primary" disabled={isPending} className="w-full">Simpan</Button>
        </form>
      ) : null}

      {matches.length === 0 ? (
        <EmptyState icon={<Trophy size={28} weight="duotone" />} title="Belum ada pertandingan" description="Buat match baru untuk mulai mencatat box score." theme="report" />
      ) : (
          ...
          {matches.map((match) => (
            <li key={match.id}>
              <Link href={`/matches/${match.id}`} className={cn('flex items-center justify-between gap-4 p-4 hover:bg-[var(--color-report-bg)]')}>
                <div>
                  <p className="font-medium text-[var(--color-report-text)]">{match.teamName} vs {match.opponent}</p>
